...
const CMS_API_URL = "http://localhost:3001/api/public";
...
export async function getPostBySlug(slug: string): Promise<Post | null> {
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (json.data) {
        ...
        } else if (json.data.slug === slug) {
          // Verify the returned post actually matches the requested slug
          if (
  ...
  // Fallback: Try to find the post in the recent posts list
  try {
